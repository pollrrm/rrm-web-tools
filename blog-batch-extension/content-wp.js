// RRM Blog BATCH Helper — Runs on the WordPress Classic Editor new/edit-post pages.
// Copied verbatim from the RRM Blog Helper extension's content-wp.js, with the
// message type and storage key changed to be BATCH-specific so this extension
// and the original RRM Blog Helper never step on each other's messages, even if
// both are enabled and both inject content scripts on the same WP pages.
// Two entry points fill the same set of fields:
//   1. chrome.storage.local "rrm_batch_pending" — reserved for a future
//      open-tab flow (not used by the side-panel walk-through today)
//   2. chrome.runtime message "RRM_BATCH_FILL_POST" — the batch side panel
//
...
(async function () {
  const { rrm_batch_pending } = await chrome.storage.local.get('rrm_batch_pending');
  if (!rrm_batch_pending || !rrm_batch_pending.payload) return;
  if (Date.now() - rrm_batch_pending.ts > 60000) {
    chrome.storage.local.remove('rrm_batch_pending');
    return;
  }

  const reloadCount = rrm_batch_pending.reloadCount || 0;
  ...
  chrome.storage.local.remove('rrm_batch_pending');
  await fillPost(rrm_batch_pending.payload, { waitForPage: true });
})();
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (!msg || msg.type !== 'RRM_BATCH_FILL_POST' || !msg.payload) return;
...
async function fillPost(p, opts = {}) {
  ...
  const ui = createProgressUI('RRM Blog Batch — Filling post…');
